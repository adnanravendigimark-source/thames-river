...
export default async function Footer() {
  ...
  return (
    <footer className="bg-[#123B27] text-white border-t border-[#0D2E1E]">
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-8 sm:py-20">
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-12 lg:gap-8">
          ...
          <div className="lg:col-span-4 space-y-4">
            ...
            />
            <div
              className="rich-content rich-content-invert max-w-sm text-xs text-[#DDE5D8]/90 leading-relaxed pt-1"
          ...
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-3 lg:col-span-5">
            {footer.columns.map((column) => (
              <div key={column.title}>
                ...
                <ul className="mt-4 space-y-2.5 text-xs text-[#DDE5D8]/80">
                  {column.links.map((link) => (
                    <li key={link.href + link.label}>
                      <Link href={link.href} className="transition hover:text-[#D6A33A]">
                        {link.label}
                      </Link>
                    </li>
                  ))}
          ...
          <div className="lg:col-span-3">
            ...
            <ul className="mt-4 space-y-3 text-xs text-[#DDE5D8]/80">
        ...
        <div className="mt-14 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-white/15 pt-8 text-xs text-[#DDE5D8]/70">
